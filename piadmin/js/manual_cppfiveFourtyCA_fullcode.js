...
$.each(cpp540coalAnalysis, function(key){   
        pathUrl += '&path=\\\\'+afServerName+'\\BALCOPOWER\\VEDANTA\\BALCO\\CPP%20-%20540MW|' + cpp540coalAnalysis[key].parameter;
});
    var batch = {
        "database":
        {
    ...
    $.when(batchResult).done(function()
    {
        ...
        $.each(batchResultItems, function (elementID) {
            ...
            var sr = "";
            $.each(cpp540coalAnalysis, function(key){  
                    if(elementName===cpp540coalAnalysis[key].parameter){
                          emName  = cpp540coalAnalysis[key].title;
                          sr = cpp540coalAnalysis[key].sr;
                    }
            });
            ...
            $.each(attrItems,function (attrID) {
                ...
                valuesID++;
                $('#tableData tbody').append("<tr><td>" + sr + "</td><td>" + emName + "</td><td>" + uom + "</td><td><input type='text' id='value" + sr + "' data-id='" + sr + "' data-WebId='" + WebId + "' value='" + attrValue + "' class='form-control input-manual WebId'></td><td><div class='status" + sr + "'></div></td></tr>")
            });           
        });
        sort_table();
    });
